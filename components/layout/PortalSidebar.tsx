    ...
    <>
      ...
      >
        ...
        <div className="h-16 flex items-center justify-between px-4 border-b border-bg-border">
          {!collapsed && (
            <Link href="/" className="font-display text-xl tracking-wider text-accent-gold">
              VIXINGO
            </Link>
          )}
          <button
        ...
        <nav className="flex-1 overflow-y-auto py-4 px-2 space-y-1">
          {isAdmin && !collapsed && (
            <p className="text-[10px] uppercase tracking-widest text-text-muted px-3 mb-2">Portal</p>
          )}
          ...
          {isAdmin && (
            <>
              {!collapsed && (
                <p className="text-[10px] uppercase tracking-widest text-text-muted px-3 mt-6 mb-2">Admin</p>
              )}
              {collapsed && <hr className="border-bg-border my-3" />}
              {adminNav.map((item) => {
                const isActive = pathname === item.href || pathname.startsWith(item.href + "/");
                return (
                  <Link
